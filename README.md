...
![pfSense](https://img.shields.io/badge/pfSense-CE%202.7.2-blue)
![OPNsense Compatible](https://img.shields.io/badge/OPNsense-24.7-orange)
![NIST](https://img.shields.io/badge/Framework-NIST%20SP%20800--41-green)
![CIS](https://img.shields.io/badge/Benchmark-CIS%20pfSense-lightgrey)
...
See `docs/FIREWALL_POLICY.md` for full rule documentation with business justification per rule.
---
...
Tuning Process:
Deployed in "Alert Only" mode for 72 hours — reviewed 1,847 would-be blocks
Identified 12 false positives (CDN ranges for legitimate services)
Added to whitelist, switched to "Deny" mode
Result: 30,000+ IOCs enforced with <0.1% false positive rate
...
Pre-patch backup — Export full config XML + package list
Validate backup integrity — Re-import to test instance (GNS3/VM)
Review patch notes — Check for breaking changes in ruleset syntax or package APIs
Apply during maintenance window — pfSense System > Update
Post-patch validation — Run connectivity checks against all VLANs and VPN
Document — Log patch version, date, validation results
Applied Updates:
...
See `docs/PATCH_MANAGEMENT.md` for full procedure and checklist.
---
...
See `docs/SIEM_INTEGRATION.md` for full decoder and rule configuration.
---
...
See `docs/COMPLIANCE_MAPPING.md` for full NIST and CIS Controls mapping.
...
Related Project: Security Operations Lab (NIDS + SIEM)
